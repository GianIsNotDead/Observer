...
if (process.argv.length === 4 && process.argv[2].match(/json|JSON/g) !== null) {
  ...
  let portSetup = new Promise(resolve => {
    EEG.getPort((ports) => {
      process.stdout.write('available ports: \n\n');
      ports.forEach((p, idx) => process.stdout.write(`${idx + 1}.${p.path}\n`));
      resolve(ports);
    });
  }).then((ports) => {
    return new Promise(resolve => {
      ...
      rl.question('\nSelect a number and press the return key: ', (n) => {
        EEG.selectPort(ports[n - 1].path, () => {
          process.stdout.write(`\nUsing port: ${ports[n - 1].path}\n`);
        });
        rl.close();
        resolve();
      });
    ...
    setTimeout(() => {
      ...
      fs.writeFile(path.resolve(`./storage/${fileName}.${fileFormat.toLowerCase()}`), JSONContent, 'utf-8', () => process.stdout.write('\nDone'));
    }, duration);
  });
}
