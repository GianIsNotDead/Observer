...
const app = uWS.App().ws('/*', {
  ...
  message: (ws, message, isBinary) => {
    ws.publish('sensors/eeg', 'hello');
    portData.port.open(() => console.log('device connected'));
    setTimeout(() => portData.requestData('device-data'), 1500);
    setTimeout(() => portData.requestData('eeg'), 3000);
    portData.port.on('data', (buff) => console.log('buff: ', buff));
    setTimeout(() => portData.stopData(), 7000);
  },
  drain: (ws) => {
